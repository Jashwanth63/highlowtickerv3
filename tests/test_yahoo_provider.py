...
def test_output_shape_matches_highlow_update_contract():
    ...
    required_keys = {"newHighs", "newLows", "lastHigh", "lastLow", "currentPrices",
...
def test_first_poll_exposes_current_prices_for_watchlist_fallback():
    provider = YahooFinanceProvider(SYMBOLS, poll_interval=90)
    fake_df = _make_df({
        "SPY":  (500.0, 502.0, 498.0, 501.0),
        "AAPL": (180.0, 182.0, 179.0, 181.0),
    ...
    assert result["currentPrices"]["SPY"] == 501.0
    assert result["currentPrices"]["AAPL"] == 181.0
    assert result["currentPrices"]["TSLA"] == 251.0


def test_nan_open_or_close_does_not_leak_nan_percent_change():
    provider = YahooFinanceProvider(["SPY", "AAPL"], poll_interval=90)
    fake_df = _make_df({
        "SPY": (math.nan, 502.0, 498.0, 501.0),
        "AAPL": (180.0, 182.0, 179.0, math.nan),
    })

    with patch("yfinance.download", return_value=fake_df):
        result = provider._poll()

    assert result["percentChange"]["SPY"] == 0.0
    assert result["percentChange"]["AAPL"] == 0.0
