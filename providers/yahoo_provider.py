    ...
    def _poll(self) -> Optional[dict]:
        ...
        for sym in self.symbols:
            try:
                ...
                raw_session_high = sym_data["High"].max()
                raw_session_low = sym_data["Low"].min()
                raw_current_price = sym_data["Close"].iloc[-1]
                raw_open_price = sym_data["Open"].iloc[0]

                session_high = self._finite_or_default(raw_session_high)
                session_low = self._finite_or_default(raw_session_low)
                current_price = self._finite_or_default(raw_current_price)
                open_price = self._finite_or_default(raw_open_price)
                bar_volume = self._finite_or_default(sym_data["Volume"].iloc[-1], default=0.0)
                ...
                has_valid_open = math.isfinite(float(raw_open_price)) and float(raw_open_price) != 0.0
                has_valid_close = math.isfinite(float(raw_current_price))
                pct = round((current_price - open_price) / open_price * 100, 2) if has_valid_open and has_valid_close else 0.0
